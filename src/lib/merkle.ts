...
import { concatBuffers } from "./utils.ts";
import { inspect } from "https://deno.land/std@0.66.0/node/util.ts";
    ...
    chunks.map(
      async ({ dataHash, minByteRange, maxByteRange }): Promise<LeafNode> => {
        return {
          ...
          id: await hash(
            await Promise.all(
              [hash(dataHash), hash(intToBuffer(maxByteRange))],
            ),
          ),
          dataHash: dataHash,
          minByteRange,
...
export async function debug(proof: Uint8Array, output = ""): Promise<string> {
  ...
  const updatedOutput = `${output}\n${inspect(Uint8Array.from(left))},${
    inspect(
      Uint8Array.from(right),
    )
